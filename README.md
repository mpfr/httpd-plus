| :warning: With the release of OpenBSD version 7.5 on Apr 5, 2024, this branch has reached its end of life and is no longer being maintained.
| --- |

# httpd-plus

Add-ons for the [OpenBSD](https://www.openbsd.org) [`httpd(8)`](http://man.openbsd.org/httpd) web server applicable to the `7.3-stable` branch.

Other branches available:
* [current](https://github.com/mpfr/httpd-plus)
* [7.4-stable](https://github.com/mpfr/httpd-plus/tree/7.4-stable)
...
Amendments:
* The info regarding the usage of the former `location-access-tests` add-on with [WordPress](https://wordpress.org) was moved from this website to [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/7.3-stable/httpd.conf.5.html#EXAMPLES).

### cache-control-headers

Optional HTTP `Cache-Control` headers via [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/7.3-stable/httpd.conf.5.html#TYPES).
...
Client address matching (`from` or `not from`) for `location` sections in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/7.3-stable/httpd.conf.5.html#location).
...
Send notification messages to UNIX-domain sockets for `server` and/or `location` sections with a `block` directive in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/7.3-stable/httpd.conf.5.html#message).
...
Simple brace expansion for `alias <name>`, `include <path>` and `location <path>` option parameters in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/7.3-stable/httpd.conf.5.html#BRACE_EXPANSION).
...
```
$ rm -rf httpd-plus-7.3-stable/
$ ftp -Vo - https://codeload.github.com/mpfr/httpd-plus/tar.gz/7.3-stable | tar xzvf -
httpd-plus-7.3-stable
httpd-plus-7.3-stable/00-updates.patch
httpd-plus-7.3-stable/01-cache-control-headers.patch
httpd-plus-7.3-stable/02-fastcgi-script-overrides.patch
httpd-plus-7.3-stable/03-client-address-filters.patch
httpd-plus-7.3-stable/04-notify-on-block.patch
httpd-plus-7.3-stable/05-brace-expansion.patch
httpd-plus-7.3-stable/LICENSE
httpd-plus-7.3-stable/README.md
httpd-plus-7.3-stable/install
```
...
```
$ doas ksh httpd-plus-7.3-stable/install 2>&1 | tee httpd-plus-install.log
...
Adapt your `httpd.conf` for the newly added features. For further information, have a look at the updated `httpd.conf(5)` [manpage](https://mpfr.net/man/httpd-plus/7.3-stable/httpd.conf.5.html) (also via `man httpd.conf`). Make sure your new configuration is valid.
