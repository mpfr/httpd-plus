...
Add-ons for the [OpenBSD](https://www.openbsd.org) [`httpd(8)`](http://man.openbsd.org/httpd) web server applicable to the `6.7-stable` branch.
...
* [Commits](https://github.com/openbsd/src/commits/master/usr.sbin/httpd) to `-current` backported to `6.7-stable` until January 27, 2021 except:
...
Optional HTTP `Cache-Control` headers via [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.7-stable/httpd.conf.5.html#TYPES).
...
Client address matching (`from` or `not from`) for `location` sections in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.7-stable/httpd.conf.5.html#location).
...
Send notification messages to UNIX-domain sockets for `server` and/or `location` sections with a `block` directive in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.7-stable/httpd.conf.5.html#message).
...
Simple brace expansion for `alias <name>`, `include <path>` and `location <path>` option parameters in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.7-stable/httpd.conf.5.html#BRACE_EXPANSION).
...
Replace the built-in error documents by customized ones provided as standalone `.html` files. Configurable via [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.7-stable/httpd.conf.5.html#errdocs) on a global or per-server basis.
...
httpd-plus-6.7-stable/05-brace-expansion.patch
httpd-plus-6.7-stable/06-custom-error-documents.patch
...
Adapt your `httpd.conf` for the newly added features. For further information, have a look at the updated `httpd.conf(5)` [manpage](https://mpfr.net/man/httpd-plus/6.7-stable/httpd.conf.5.html) (also via `man httpd.conf`). Make sure your new configuration is valid.
