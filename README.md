...
Add-ons for the [OpenBSD](https://www.openbsd.org) [`httpd(8)`](http://man.openbsd.org/httpd) web server applicable to the `6.9-stable` branch.
...
* [Commits](https://github.com/openbsd/src/commits/master/usr.sbin/httpd) to `-current` backported to `6.9-stable` until October 15, 2021

### cache-control-headers

Optional HTTP `Cache-Control` headers via [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.9-stable/httpd.conf.5.html#TYPES).
...
Client address matching (`from` or `not from`) for `location` sections in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.9-stable/httpd.conf.5.html#location).
...
Send notification messages to UNIX-domain sockets for `server` and/or `location` sections with a `block` directive in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.9-stable/httpd.conf.5.html#message).

This cooperates perfectly with [pftbld(8)](https://github.com/mpfr/pftbld/tree/6.9-stable), offering an easy and straightforward means to effectively protect the web server from offensive clients and successively build customized firewall blocklists. In the example below, access to `/restricted*` URLs from outside the `10.0.0/24` network is not just blocked, but `httpd(8)` also reports client IP addresses to `pftbld(8)` (with its listening socket at `/var/www/run/pftbld-www.sock`) for further handling.
...
Simple brace expansion for `alias <name>`, `include <path>` and `location <path>` option parameters in [`httpd.conf(5)`](https://mpfr.net/man/httpd-plus/6.9-stable/httpd.conf.5.html#BRACE_EXPANSION).
...
```
$ rm -rf httpd-plus-6.9-stable/
$ ftp -Vo - https://codeload.github.com/mpfr/httpd-plus/tar.gz/6.9-stable | tar xzvf -
httpd-plus-6.9-stable
httpd-plus-6.9-stable/00-updates.patch
httpd-plus-6.9-stable/01-cache-control-headers.patch
httpd-plus-6.9-stable/02-fastcgi-script-overrides.patch
httpd-plus-6.9-stable/03-client-address-filters.patch
httpd-plus-6.9-stable/04-notify-on-block.patch
httpd-plus-6.9-stable/05-brace-expansion.patch
httpd-plus-6.9-stable/06-custom-error-documents.patch
httpd-plus-6.9-stable/LICENSE
httpd-plus-6.9-stable/README.md
httpd-plus-6.9-stable/install
```
...
```
$ doas ksh httpd-plus-6.9-stable/install 2>&1 | tee httpd-plus-install.log
...
Adapt your `httpd.conf` for the newly added features. For further information, have a look at the updated `httpd.conf(5)` [manpage](https://mpfr.net/man/httpd-plus/6.9-stable/httpd.conf.5.html) (also via `man httpd.conf`). Make sure your new configuration is valid.
